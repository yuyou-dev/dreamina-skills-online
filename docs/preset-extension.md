...
- 输入字段：必填项和默认值
- 工具链：即梦在线工具的调用顺序
- 默认模型：图片任务默认 4.7 模型 2k 版，视频任务默认 `seedance2.0fast_vip`；用户指定模型版本时按指定
- 提示词模板：图片、视频或设计矩阵模板
- 质量检查：通过标准和失败重试策略
...
```text
text2image 生成产品母图后，立即把母图作为 @图片1 交给 image2image 生成槽位图。
image2image 生成主图、微距图、佩戴图后，立即把它们作为 @图片1-@图片3 交给 multi_modal2video。
multi_modal2video 生成片段后，立即交给 clip_join 输出最终成片。
```
