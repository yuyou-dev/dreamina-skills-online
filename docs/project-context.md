...
Installable Skills use the generic official tool names only. Platform-specific aliases are maintenance concerns and are not written into the runtime files.

Parameter precedence is user request > panel/session defaults > Skill-declared defaults > tool constraints > general fallback. Skill-declared model defaults apply only when the user and current environment have not already selected a model.

Skill-declared model defaults:

- Video tasks use `seedance2.0fast_vip` by default.
- Image tasks use the 4.7 model 2k version by default.
- If the user or current panel specifies a model version, use it instead of the Skill default.
...
- generated segments should continue into `clip_join`;
...
- Add more jewelry design presets into `projects/jewelry-design/skill/专业珠宝设计技能.md` directly when they are core.
