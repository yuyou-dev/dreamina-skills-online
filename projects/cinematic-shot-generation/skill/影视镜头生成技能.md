---
name: 影视镜头生成技能
description: 把一句影视需求或参考素材转成可控分镜、关键帧、Seedance视频和质量修复方案，适用于广告、剧情、产品、空间漫游、动作、POV、科幻、纪录片及UGC镜头。用户要求连续影视镜头或分镜转视频时使用；不用于纯静态海报、纯文案或以大批量任务管理为核心的请求。
---
...
`generate_form_for_info_collection` 只收集文字、单选和多选信息；图片、视频、音频、Logo 或其他附件必须由用户在对话中上传或使用当前会话已有资源，不得通过表单收集。
...
- 当前面板或会话已有模型设置时优先继承；没有时才使用上述技能默认模型。
- 当前面板或会话未设置图片模型时，不得把本技能的 4.7 默认值自动升级为 5.0、最新模型或自动推荐模型。
- Seedance 2.0 系列单次视频生成最长 15 秒；默认先生成 1 条完整单条视频，把节奏、镜头和信息密度压缩进 15 秒内。只有用户明确要求多段、超长成片或剪辑合成时，才规划多条视频并用 `clip_join` 拼接。
...
| `clip_join` | 用户明确要求多段、片头片尾、字幕、裁切、配乐或成片剪辑时使用 | 单条视频通过 QA 且无需剪辑时可直接交付 |
| `image_super_resolution` | 高清化已通过身份与结构检查的分镜图或关键帧 | 高清图继续作为视频参考图 |

视频素材纪律：

- 参考图、关键帧和视频片段生成阶段不生成 BGM；只有进入最终 `clip_join` 且用户明确要求配乐时才加入。
- 分镜或 Clip 计划确认前不单独追问画幅；即将生成参考素材或视频且画幅仍缺失时，再通过表单补齐。
- 调用 `multi_modal2video` 时，普通素材按输入顺序使用 `@图片N`、`@视频N`、`@音频N`；用户参考主体必须写成 `<subject>主体名称</subject>`，并把对应真实 `resource_id` 写入图片输入参数。

在线版适配：

- 当前面板或会话未设置图片模型时，在图片工具中显式选择 `seedream4.7` / 4.7，不使用会静默升级模型的入口。
- 如工具界面存在多个图片入口，使用通用 `text2image` / `image2image`，并按参数优先级选择模型。
...
- 只有用户明确要求多段长片、多个平台版本、片头片尾、配乐字幕或后期合成时，才使用 `clip_join`。
...
| CM-FINAL | 可选剪辑成片 | clip_join |  | CM-V01 或用户指定的多段素材 | optional | final_delivery |
```
...
- 需要片头、字幕、裁切、配乐、品牌尾卡、比例适配或用户明确要求多段拼接时，必须进入 `clip_join` 后再交付。
...
- 用户明确要求多段或需要剪辑时已调用 `clip_join`，不是只交付素材列表。
...
未通过时：回到对应失败视频、Seedance prompt 或 `clip_join` 设置修复，不得标记最终交付完成。

中间态处理：

- 只有 `SB-REF-01`：继续生成 `CM-SEEDANCE-PROMPT`。
- 只有 `CM-SEEDANCE-PROMPT`：继续调用视频工具。
- 只有 `VID-01` 但用户需要剪辑或多段：继续调用 `clip_join`。
- 任一中间态都不得输出“已完成”。
...
- 高风险动作连续失败：先改为单条 5-8 秒高可控版本；只有用户明确要求长片或多段剪辑时，才拆成多个视频任务后进入 `clip_join`。
