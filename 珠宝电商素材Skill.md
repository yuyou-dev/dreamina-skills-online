...
链式调用要求：每一步都必须写明下一步工具和输入素材。图片生成完成后，继续用生成图作为参考进入视频工具；视频片段生成后，继续进入 `video_editor`；缺少必填信息时才调用表单，不把可选项变成阻塞。
...
| 视觉风格 | 否 | 默认高端真实商业摄影 |
| 输出背景 | 否 | 款式母图和白底主图默认白底；只有佩戴图、生活方式图、包装图、广告图等槽位使用场景背景 |
...
- 产品身份锁。
- 当前槽位目的。
- 输入角色：上传产品图、生成母图、设计图、草图或参考图。
- 保留项：产品轮廓、主石颜色与琢型、镶嵌结构、金属材质、扣具/耳针/吊环/戒臂等。
- 允许变化项：仅限当前槽位需要的角度、光线、背景、道具、佩戴环境或细节放大。
- 构图、背景、灯光、比例。
- 禁止改变产品结构。
...
成图质量:
专业珠宝商业摄影或高精度产品渲染，主体清晰，边缘干净，真实金属反光，宝石火彩明确，轻微自然投影。
白底主图、母图和标准化产品图必须使用纯白或极浅灰白背景；除非当前槽位是佩戴、生活方式、包装或广告氛围，不使用场景背景。

禁止:
改变产品造型、宝石颜色、镶嵌结构、金属材质；禁止假 logo、证书、价格、文字水印、额外首饰、产品变形、复杂背景抢主体。
